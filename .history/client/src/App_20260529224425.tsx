...
export interface CharacterConfig {
  ...
  faction?: string;
  /**
   * Worge shapeshifter forms. The base `modelPath` is the human mage form —
   * mage clothing with a nature staff, used for ranged casting and healing.
   * Setting either of these enables an attack-form toggle:
   *   - CLASS_ABILITY_1 (KeyE): wolf form  — fast melee predator
   *   - CLASS_ABILITY_3 (KeyX): bear form  — heavy melee bruiser
   * Transforms are plain model-path swaps routed through useCharacterController,
   * which reloads the GLB through the standard pipeline. No skinned-mesh morph
   * targets are involved, so the animation mixer never has to blend two
   * incompatible rigs at once.
   *
   * `worgeFormModelPath` is kept as the legacy alias for the bear form so
   * existing saves/data keep working; new code should prefer the explicit
   * `worgeBearFormModelPath` field.
   */
  worgeFormModelPath?: string | null;
  worgeBearFormModelPath?: string | null;
  worgeWolfFormModelPath?: string | null;
  /**
   * Optional per-form visual-height multiplier applied on top of the
   * character's base `scale`. Lets the wolf read as smaller/faster and the
   * bear as bulkier without needing distinct GLBs for each silhouette.
   */
