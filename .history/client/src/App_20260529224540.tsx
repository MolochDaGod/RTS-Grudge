...
function PlayerModel({
  ...
  useEffect(() => {
    ...
    if (pts.length >= 12) {
      ...
      console.warn("[Player] Hull sampling produced too few points; staying on capsule fallback. - App.tsx:541");
      setHullPoints(null);
    }
  ...
  useEffect(() => {
    ...
    if (!rightHand.current && !leftHand.current) {
      console.warn(`[Player] No hand bones found on model (skeleton: ${skelType}, bones: ${boneNames.length}) - App.tsx:636`);
    }
    ...
    const attachToHand = (
      ...
      if (weaponModelId) {
        loadWeaponModel(data.type, weaponModelId).then((modelGroup) => {
          ...
          if (cancelled) return;
          console.warn(`[Player] Weapon model load failed for ${weaponModelId}, using procedural fallback: - App.tsx:662`, err);
  ...
  useEffect(() => {
    ...
    loadAsset(entry.path, "medium", "back_accessory").then((gltf) => {
      ...
      if (!cancelled) {
        console.warn(`[Player] Back accessory load failed for ${accId}: - App.tsx:769`, err);
      }
    });
  ...
  const tryDamageEnemies = useCallback((damageAmount: number, range: number = ATTACK_RANGE, combatState?: string) => {
    ...
    try {
      ...
      console.warn("[Player] Destructibles damage failed: - App.tsx:1103", err);
    }
    ...
    for (let i = 0; i < maxTargets; i++) {
      ...
      if (killed) {
        ...
        try {
          ...
          console.warn("[Player] Build system resource add failed: - App.tsx:1264", err);
        }
