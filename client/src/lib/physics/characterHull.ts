...
export function sampleCharacterPoints(
  ...
  // Per-mesh stats — flushed at the end so the diagnostic shows exactly
  // which meshes contributed (and which were skipped + why) when a tiny
  // hull leaks through. Cheap because we only stringify if logging is on.
  const debug = (root.userData as any)?.__hullDebug === true
    || (typeof window !== "undefined" && (window as any).__HULL_DEBUG === true);
  const stats: Array<{ name: string; verts: number; reason?: string }> = [];

  // Walk parent chain for visibility — three.js doesn't propagate
  // `visible=false` to children's `visible` flag, only to render. Hidden
  // alt-meshes / LOD variants / equipment-slot stubs would otherwise
  // bloat the hull silently.
  const isInvisibleChain = (o: THREE.Object3D | null): boolean => {
  ...
  root.traverse((obj) => {
    ...
    if (isInvisibleChain(mesh)) {
      if (debug) stats.push({ name: mesh.name, verts: 0, reason: "hidden" });
      return;
    }
    if ((mesh.userData as any)?.skipCollider === true) {
      if (debug) stats.push({ name: mesh.name, verts: 0, reason: "skipCollider" });
      return;
    }
    if (ignoreNames && (ignoreNames.test(mesh.name) || ignoreNames.test(mesh.parent?.name ?? ""))) {
      if (debug) stats.push({ name: mesh.name, verts: 0, reason: "name-regex" });
      return;
    }
    const mat = Array.isArray(mesh.material) ? mesh.material[0] : mesh.material;
    if (mat && ignoreMaterialNames && ignoreMaterialNames.test(mat.name ?? "")) {
      if (debug) stats.push({ name: mesh.name, verts: 0, reason: "material-regex" });
      return;
    }
    const geom = mesh.geometry as THREE.BufferGeometry | undefined;
    const pos = geom?.attributes?.position as THREE.BufferAttribute | undefined;
    if (!pos || pos.count < 4) {
      if (debug) stats.push({ name: mesh.name, verts: pos?.count ?? 0, reason: "too-few-verts" });
      return;
    }
    ...
    if (debug) stats.push({ name: mesh.name + (skinned ? " [skinned]" : ""), verts: added });
  });

  if (debug) {
    const kept = stats.filter((s) => !s.reason);
    const skipped = stats.filter((s) => s.reason);
    // Compute AABB of the assembled cloud so we can spot a collapsed
    // hull at a glance — a healthy humanoid is roughly 1.8 m tall and
    // 0.5 m wide. Anything sub-meter is the "tiny diamond" bug.
    ...
    console.log(
      `[hull] root=${root.name || "<unnamed>"} pts=${pts.length / 3} aabb=${dx}x${dy}x${dz}`,
      { kept, skipped },
    );
  }
