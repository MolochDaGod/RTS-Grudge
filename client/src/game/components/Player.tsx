...
import { useInventory, INVENTORY_EQUIP_MAP } from "@/lib/stores/useInventory";
...
function PlayerModel({
  ...
  // mesh bounds — never from caller-supplied scale metadata. Source GLBs
  ...
  useEffect(() => {
    ...
    //  - <primitive object={scene} /> has actually attached `scene` to
    //    `modelRef` (the parent chain is needed for matrixWorld math),
    //  - the AnimationMixer has ticked at least once and skinned bones
    //    have a real `matrixWorld` (otherwise applyBoneTransform skins
    //    every vertex toward the origin → 4-vert "diamond" hull).
    // Without this, the effect races the first render and we ship a
    // collapsed point cloud that QuickHull dedupes down to a tetrahedron.
    let cancelled = false;
    const handle = requestAnimationFrame(() => {
      ...
      // Sanity-check the AABB. A real humanoid is ~1.8 m tall and
      // ≥ 0.4 m on the horizontal axes. If we're below those thresholds
      // the sample collapsed (skeleton not bound, all meshes skipped,
      // morph zeroed a bone) — fall back to the capsule rather than
      // shipping a sub-meter collider that the player floats on top of.
      ...
      const dz = maxZ - minZ;
      const ok =
        pts.length / 3 >= MIN_POINTS &&
        dy >= MIN_HEIGHT &&
        Math.max(dx, dz) >= MIN_WIDTH;

      if (ok) {
        setHullPoints(pts);
      } else {
        console.warn(
          `[Player] Hull sampling produced a degenerate cloud (pts=${pts.length / 3}, aabb=${dx.toFixed(2)}x${dy.toFixed(2)}x${dz.toFixed(2)}); staying on capsule fallback. Set window.__HULL_DEBUG=true and reload to see per-mesh contributions.`,
        );
        setHullPoints(null);
      }
    });
  ...
  // Safety: clear the boat-attached window latch whenever the player's
  // RigidBody re-keys (character swap, hull rebuild, save load). The flag
  // is a `window` global so it survives React remounts; if a save was
  // captured while attached to a beached boat, loading it would otherwise
  // leave the player permanently glued to the stern with zero gravity.
  // BeachedBoats.tsx also clears the flag from its own useFrame when its
  // attach ref is null — this is belt-and-suspenders for the case where
  // BeachedBoats hasn't mounted yet on the new scene.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (/^Digit[0-9]$/.test(e.code)) {
        ...
        // Tools/weapons/armor: auto-equip on hotbar press so picking the
        // shovel (or any gear) actually moves it into `equipped.mainHand`
        // — that's what TerrainEditor and the combat/anim pipelines key
        // off. Without this, selectSlot only highlights the icon and the
        // shovel never activates the heightfield editor.
        if (item && INVENTORY_EQUIP_MAP[item.id]) {
          inv.equipFromInventory(item.id);
        }
