...
const RAFT_PATH = "/models/raft/raft.glb";
...
function generateCannonballs(): Cannonball[] {
  ...
  const playerShipPos = new THREE.Vector3(0, 2, 0);
  let id = 0;
...
function IntroRaft({
  ...
  // `height` is the final visible Y-extent of the raft (deck + mast). The
  // pipeline normalizes the model to this height, then we render at scale=1
  // so the value passed in is the value you see.
  const model = useIntroModel(RAFT_PATH, height);
  const groupRef = useRef<THREE.Group>(null);

  useFrame(() => {
    if (!groupRef.current) return;
    const t = introState.time;
    // Rafts bob more than ships - lighter and more affected by waves
    groupRef.current.position.y = position[1] + Math.sin(t * 1.4 + position[0] * 0.1) * 0.25 + Math.sin(t * 0.7) * 0.1;
    groupRef.current.rotation.z = Math.sin(t * 1.1 + position[0]) * 0.08;
    groupRef.current.rotation.x = Math.sin(t * 0.9 + 0.4) * 0.05;
  });
...
function ThrownCharacter() {
  ...
  const throwOrigin = useMemo(() => new THREE.Vector3(0, 3, 0), []);
  ...
  const idleStandPos = useMemo(() => new THREE.Vector3(0, 1.2, 0), []);
  ...
  useFrame((_, delta) => {
    ...
    if (t < TIMELINE.grabPlayer) {
      // Standing on the raft, subtle weight-shift sway so it doesn't look static
      const sway = Math.sin(t * 1.2) * 0.04;
      groupRef.current.position.set(sway, 1.2, 0);
...
function CinematicCamera() {
  ...
  useFrame(() => {
    ...
    if (t < TIMELINE.firstVolley) {
      ...
      const charX = p * 4;
      const charZ = -p * 18;
      lookAt.current.set(charX, 2 + Math.sin(p * Math.PI) * 3, charZ);
...
function IntroScene3D({ onUpdate }: { onUpdate: () => void }) {
  return (
    <>
      ...
      <group>
        <IntroRaft position={[0, 0.2, 0]} rotationY={Math.PI * 0.5} height={2.4} />
