...
import { syncObjectStore } from "./grudge";
...
const AI_PROVIDERS: AIProviderDef[] = [
  {
    ...
    getKey: () => process.env.OPENROUTER_API_KEY || process.env.OPENROUTER_KEY || "",
  ...
  },
  {
    ...
    getKey: () => process.env.MEGALLM_API_KEY || "",
    models: [
  ...
  },
  {
    ...
    getKey: () => process.env.AGENTROUTER_API_KEY || "",
    models: [
  ...
  },
  {
    ...
    getKey: () => process.env.ROUTEWAY_API_KEY || "",
    models: [
...
const GAME_SYSTEM_PROMPT = `You are VIBE, an AI game development assistant embedded in a 3D survival/combat RPG built with React Three Fiber, Rapier WASM physics, XState v5, and the Grudge Warlords stat system.

You help users create, edit, debug, and enhance their game. You have DEEP knowledge of this project's source code and can make live edits.
...
LIVE EDITING:
When the user asks you to make changes, you can return file edit commands in this format:
\`\`\`vibe-edit
...
RESPONSE FORMAT:
- When modifying game config, respond with JSON patches: {"section": "terrain|combat|physics|etc", "changes": {...}}
- When editing source code, use vibe-edit blocks
- When debugging, explain the issue AND provide the fix
- Be concise but thorough. Use game dev terminology.
- For code generation, use TypeScript with React Three Fiber patterns.
- Suggest improvements, catch balance issues, think about player experience.`;
